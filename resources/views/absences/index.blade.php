                    ...
                    @forelse($absences as $absence)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full" src="{{ $absence->user->profile_photo_url }}" alt="{{ $absence->user->name }}">
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $absence->start_date->format('M d, Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="{{ route('absences.show', $absence) }}" class="text-blue-600 hover:text-blue-900">View</a>
                                    @if($absence->user_id === auth()->id() && $absence->status === 'pending')
                                        <a href="{{ route('absences.edit', $absence) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
