...
<div class="bg-white shadow-sm rounded-lg">
    <div class="p-6 border-b border-gray-200">
        <div class="mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Edit Payment</h2>
            <p class="text-gray-600">Update payment details below</p>
        </div>
        ...
        <form action="{{ route('payments.update', $payment) }}" method="POST" id="payment-form">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select id="payment_type" name="payment_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                        ...
                        <option value="trainee" {{ $payment->payment_type == 'trainee' ? 'selected' : '' }}>Trainee Payment</option>
                        <option value="supplier" {{ $payment->payment_type == 'supplier' ? 'selected' : '' }}>Supplier Payment</option>
                ...
                <div id="trainee-selection" class="{{ $payment->payment_type != 'trainee' ? 'hidden' : '' }}">
                    ...
                    <select id="student_id" name="student_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                        ...
                        <option value="1" {{ $payment->user_id == 1 ? 'selected' : '' }}>John Doe (ID: T001)</option>
                        <option value="2" {{ $payment->user_id == 2 ? 'selected' : '' }}>Jane Smith (ID: T002)</option>
                        <option value="3" {{ $payment->user_id == 3 ? 'selected' : '' }}>Michael Johnson (ID: T003)</option>
                    </select>
                </div>

                <div>
                    <label for="amount" class="block text-sm font-medium text-gray-700 mb-1">Amount (DH)</label>
                    <input type="number" id="amount" name="amount" step="0.01" min="0" value="{{ old('amount', $payment->amount) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                    @error('amount')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="payment_date" name="payment_date" value="{{ old('payment_date', $payment->payment_date->format('Y-m-d')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                    @error('payment_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="due_date" name="due_date" value="{{ old('due_date', $payment->due_date ? $payment->due_date->format('Y-m-d') : '') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                    @error('due_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>{{ old('description', $payment->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="border-t border-gray-200 pt-4 mb-4">
                ...
                <div id="payment-items">
                    @forelse($payment->items as $index => $item)
                        <div class="payment-item grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-3 border border-gray-200 rounded-md">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                <input type="text" name="items[{{ $index }}][description]" value="{{ $item->description }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Amount (DH)</label>
                                <input type="number" name="items[{{ $index }}][amount]" value="{{ $item->amount }}" step="0.01" min="0" class="item-amount w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                                <select name="items[{{ $index }}][category]" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                                    <option value="tuition" {{ $item->category == 'tuition' ? 'selected' : '' }}>Tuition Fee</option>
                                    <option value="materials" {{ $item->category == 'materials' ? 'selected' : '' }}>Learning Materials</option>
                                    <option value="activities" {{ $item->category == 'activities' ? 'selected' : '' }}>Activities</option>
                                    <option value="other" {{ $item->category == 'other' ? 'selected' : '' }}>Other</option>
                                </select>
                            </div>
                            <div class="flex items-end">
                                <button type="button" class="remove-item text-red-500 hover:text-red-700 mt-4" {{ $index == 0 && count($payment->items) == 1 ? 'style=display:none' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let itemCounter = {{ count($payment->items) > 0 ? count($payment->items) : 1 }};
