...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-3xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Edit Academic Term</h1>
        ...
        <form action="{{ route('terms.update', $term) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="semester" {{ old('type', $term->type) == 'semester' ? 'selected' : '' }}>Semester</option>
                        <option value="trimester" {{ old('type', $term->type) == 'trimester' ? 'selected' : '' }}>Trimester</option>
                        <option value="quarter" {{ old('type', $term->type) == 'quarter' ? 'selected' : '' }}>Quarter</option>
                    </select>
                </div>

                <div>
                    <label for="academic_year" class="block text-sm font-medium text-gray-700 mb-1">Academic Year</label>
                    <input type="text" name="academic_year" id="academic_year" value="{{ old('academic_year', $term->academic_year) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $term->start_date->format('Y-m-d')) }}" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                </div>

                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $term->end_date->format('Y-m-d')) }}" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="registration_deadline" id="registration_deadline" 
                        value="{{ old('registration_deadline', $term->registration_deadline->format('Y-m-d')) }}" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                </div>

                <div>
                    <label for="drop_deadline" class="block text-sm font-medium text-gray-700 mb-1">Drop Deadline</label>
                    <input type="date" name="drop_deadline" id="drop_deadline" 
                        value="{{ old('drop_deadline', $term->drop_deadline->format('Y-m-d')) }}" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                </div>

                <div>
                    <label for="grading_deadline" class="block text-sm font-medium text-gray-700 mb-1">Grading Deadline</label>
                    <input type="date" name="grading_deadline" id="grading_deadline" 
                        value="{{ old('grading_deadline', $term->grading_deadline->format('Y-m-d')) }}" required
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                </div>
